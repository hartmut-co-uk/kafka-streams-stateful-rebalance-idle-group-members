...
public class Util implements AutoCloseable {
    ...
    public class Randomizer implements AutoCloseable, Runnable {
        ...
        public Randomizer(Properties producerProps, String topic) {
            ...
            this.props.putAll(producerProps);
            this.props.setProperty(ProducerConfig.CLIENT_ID_CONFIG, "faker-" + UUID.randomUUID());
        }
        ...
        public void run() {
            while (!closed) {
                try (KafkaProducer<String, String> producer = new KafkaProducer(props)) {
                    ...
                    while (!closed) {
                        try {
                            logger.info("produce 10 new random string messages...");
                            for (int i = 0; i < 10; i++) {
    ...
    public Randomizer startNewRandomizer(Properties producerProps, String topic) {
        logger.info("Starting random string data producer");
